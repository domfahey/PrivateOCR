...
- **Context Menu OCR**: Right-click any image on a webpage and select "Extract text with PrivateOCR" to run OCR directly on it.
- **Clipboard Paste**: Click the new "Paste" button or press Ctrl+V to OCR an image from your clipboard.
- **File Upload**: Drag-and-drop images onto the new drop zone, or click "Choose File" to select an image file for OCR.
- **New Permissions**: Added `contextMenus` and `clipboardRead` permissions to support new features.

### Changed

- **UI Layout**: Added import section with drop zone below the capture buttons.
